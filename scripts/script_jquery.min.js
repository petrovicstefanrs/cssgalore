$(document).ready(function() {
  ...
  function l() {
    var b = $("#box_shadow_horizontal_length").val(), a = $("#box_shadow_vertical_length").val(), d = $("#box_shadow_blur_radius").val(), c = $("#box_shadow_spread").val(), h = $("#box_shadow_position").val();
    "outset" == h && (h = "");
    var e = $("#box_shadow_opacity").val();
    1 < e && (e = 1);
    0 > e && (e = 0);
    if ("hex" == $("#box_shadow_color_type").val()) {
      $("#box_shadow_opacity").prop("disabled", !0), e = $("#box_shadow_color").val();
    } else {
      $("#box_shadow_opacity").prop("disabled", !1);
      var g;
      g = $("#box_shadow_color").val();
      g = (g = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(g)) ? {r:parseInt(g[1], 16), g:parseInt(g[2], 16), b:parseInt(g[3], 16)} : null;
      e = "rgba(" + g.r + "," + g.g + "," + g.b + "," + e + ")";
    }
    b || (b = 0);
    a || (a = 0);
    d || (d = 0);
    c || (c = 0);
    !b && !a || 0 == b && 0 == a ? ($("#generated_code_text").html("There is no generated code"), $(".box_shadow_preview").css({"-moz-box-shadow":"none", "-webkit-box-shadow":"none", "box-shadow":"none"})) : ($("#generated_code_text").html("<code><span>-moz-box-shadow:" + h + " " + b + "px " + a + "px " + d + "px " + c + "px " + e + ";</span></br><span>-webkit-box-shadow:" + h + " " + b + "px " + a + "px " + d + "px " + c + "px " + e + ";</span></br><span>box-shadow:" + h + " " + b + "px " + a + 
    "px " + d + "px " + c + "px " + e + ";</span></code>"), $(".box_shadow_preview").css({"-moz-box-shadow":h + " " + b + "px " + a + "px " + d + "px " + c + "px " + e, "-webkit-box-shadow":h + " " + b + "px " + a + "px " + d + "px " + c + "px " + e, "box-shadow":h + " " + b + "px " + a + "px " + d + "px " + c + "px " + e}));
  }
  function m() {
    var b = $("#transform_scale_x").val(), a = $("#transform_scale_y").val(), d = $("#transform_translate_x").val(), c = $("#transform_translate_y").val(), h = $("#transform_skew_x").val(), e = $("#transform_skew_y").val(), g = $("#transform_rotate").val();
    b || (b = 1);
    a || (a = 1);
    d || (d = 0);
    c || (c = 0);
    ...
    e || (e = 0);
    g || (g = 0);
    var f = "scaleX(" + b + ") scaleY(" + a + ") rotate(" + g + "deg) translateX(" + d + "px) translateY(" + c + "px) skewX(" + h + "deg) skewY(" + e + "deg)";
    $(".transform_preview").css({"-moz-transform":f, "-webkit-transform":f, transform:f});
    !(b || a || d || c || h || e || g) || 1 == b && 1 == a && 0 == d && 0 == c && 0 == h && 0 == e && 0 == g ? $("#generated_code_text").html("There is no generated code") : (b && 1 != b || (f = f.replace(/scaleX\([a-zA-Z0-9\.]+\)/g, "")), a && 1 != a || (f = f.replace(/scaleY\([a-zA-Z0-9\.]+\)/g, "")), d && 0 != d || (f = f.replace(/translateX\([a-zA-Z0-9\.]+\)/g, "")), c && 0 != c || (f = f.replace(/translateY\([a-zA-Z0-9\.]+\)/g, "")), h && 0 != h || (f = f.replace(/skewX\([a-zA-Z0-9\.]+\)/g, 
    "")), e && 0 != e || (f = f.replace(/skewY\([a-zA-Z0-9\.]+\)/g, "")), g && 0 != g || (f = f.replace(/rotate\([a-zA-Z0-9\.]+\)/g, "")), $("#generated_code_text").html("<code><span>-moz-transform:" + f + ";</span></br><span>-webkit-transform:" + f + ";</span></br><span>transform:" + f + ";</span></code>"), $(".transform_preview").css({"-moz-transform":f, "-webkit-transform":f, transform:f}));
  }
  function k() {
    var b = $("#transition_opacity_timing").val(), a = $("#transition_opacity_opacity").val(), d = $("#transition_opacity_duration").val(), c = $("#transition_opacity_delay").val(), h = $("#transition_height_timing").val(), e = $("#transition_height_height").val(), g = $("#transition_height_duration").val(), f = $("#transition_height_delay").val(), k = $("#transition_width_timing").val(), l = $("#transition_width_width").val(), m = $("#transition_width_duration").val(), q = $("#transition_width_delay").val(), 
    n = $("#transition_bgcolor_timing").val(), p = $("#transition_bgcolor_color").val(), t = $("#transition_bgcolor_duration").val(), r = $("#transition_bgcolor_delay").val();
    d || (d = 0);
    c || (c = 0);
    a && 1 != a || (a = !1);
    1 < a && (a = 1);
    ...
    t || (t = 0);
    r || (r = 0);
    e ? (g = "height " + g + "s " + h + " " + f + "s", h = "height: " + e + "px;") : h = g = "";
    l ? (m = "width " + m + "s " + k + " " + q + "s", k = "width: " + l + "px;") : k = m = "";
    a ? (d = "opacity " + d + "s " + b + " " + c + "s", b = "opacity: " + a + ";") : b = d = "";
    w ? (n = "background-color " + t + "s " + n + " " + r + "s", p = "background-color: " + p + ";") : p = n = "";
    n = (g + ", " + m + ", " + d + ", " + n).replace(/^([\,\s]+)|(\s\,\W)|([\,\s]+)$/g, " ");
    p = h + k + b + p;
    t = ".transition_preview{ -moz-transition:" + n + ";-webkit-transition:" + n + ";o-transition:" + n + ";transition:" + n + ";} .transition_preview:hover{ " + p + " }";
    p = p.replace(/\;(?=[^;]*\;)/g, ";</br>");
    v.childNodes[0] ? (v.childNodes[0].nodeValue = t, console.log(t)) : v.appendChild(document.createTextNode(t));
    e || l || a || w ? $("#generated_code_text").html("<code>.selector{</br><span>-moz-transition:" + n + ";</span></br><span>-webkit-transition:" + n + ";</span></br><span>-o-transition:" + n + ";</span></br><span>transition:" + n + ";</span></br>}</code></br><code>.selector:hover{</br><span>" + p + "</span></br>}</code>") : $("#generated_code_text").html("There is no generated code");
  }
  $(".button-collapse").sideNav();
  var y = new Clipboard(".copybtn");
